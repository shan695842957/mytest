...
interface BMSBauTabLevel3Props {
  instanceId: number
...
export function BMSBauTabLevel3({
  instanceId,
  realtimeValues = {},
  onFaultReset,
}: BMSBauTabLevel3Props) {
  const { t, i18n } = useTranslation('bms')

  // 获取 BAU 页面字段配置
  const { data: fieldConfigsData, isLoading } = useQuery({
    queryKey: ['bms-field-configs', instanceId, 'BAU'],
    queryFn: () => getBMSFieldConfigList(instanceId, 'BAU'),
    enabled: !!instanceId,
  })
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="text-center text-muted-foreground">
            {t('bms.bau.no_fields_configured', '暂无字段配置，请在配置页面添加字段')}
          </div>
        </CardContent>
      </Card>
    )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-xs text-muted-foreground">
        {t('bms.bau.no_realtime_tip', '实时值依赖 WebSocket 数据，未收到时显示空值。')}
      </div>
    </div>
  )
}
