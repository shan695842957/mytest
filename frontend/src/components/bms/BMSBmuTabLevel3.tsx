...
interface BMSBmuTabLevel3Props {
  instanceId: number
  // WebSocket 缓存数据（key: field_key, value: 实时值）
  realtimeValues?: Record<string, any>
}

export function BMSBmuTabLevel3({
  instanceId,
  realtimeValues = {},
}: BMSBmuTabLevel3Props) {
  ...
  // 根据层级配置生成包列表（虚拟ID，每个簇的包数相同）
  const packs = useMemo(() => {
    if (!hierarchyConfig || !selectedClusterId) return []
    const packCount = hierarchyConfig.pack_count_per_cluster || 0
    return Array.from({ length: packCount }, (_, i) => ({
      id: `${selectedClusterId}-pack-${i + 1}`,
      number: i + 1,
      cluster_id: selectedClusterId,
    }))
  }, [hierarchyConfig, selectedClusterId])
  ...
  // 获取字段显示值
  const getFieldValue = (field: BMSFieldConfig, clusterId?: string, packId?: string): string | number | boolean | null => {
    // TODO: 根据clusterId和packId获取对应包的实时值
  ...
  // 格式化字段值显示
  const formatFieldValue = (field: BMSFieldConfig, clusterId?: string, packId?: string): string => {
    const value = getFieldValue(field, clusterId, packId)
  ...
  return (
    <div className="space-y-6">
      {/* 簇和包选择器 */}
      {(clusters.length > 0 || packs.length > 0) && (
        <Card>
          <CardHeader>
                  ...
                  {sortedFields.map((field) => (
                    <Card key={field.id} className="p-4">
                      ...
                      <div className="text-lg font-bold">
                        {formatFieldValue(field, selectedClusterId, selectedPackId)}
                      </div>
                    </Card>
                  ))}
                </div>
