...
import { useDeleteCommInstance } from '@/hooks/useCommInstanceQueries'

interface DeleteCommInstanceDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  instance: CommInstance | null
}

export function DeleteCommInstanceDialog({
  open,
  onOpenChange,
  instance,
}: DeleteCommInstanceDialogProps) {
  const { t } = useTranslation('config')
  const deleteMutation = useDeleteCommInstance()
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{t('commInstance.success.deleted')}</AlertDialogTitle>
          <AlertDialogDescription>
