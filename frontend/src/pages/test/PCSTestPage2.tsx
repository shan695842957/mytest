/**
 * PCS 展示页面（版本2 - 使用EnergyFlowPanel组件）
...
export default function PCSTestPage2() {
  ...
  // 计算效率
  const efficiency = data.acActivePower !== 0 
    ? ((Math.abs(data.dcPower) / Math.abs(data.acActivePower)) * 100)
    : 0
  
  // 计算转换损耗
  const conversionLoss = Math.abs(Math.abs(data.acActivePower) - Math.abs(data.dcPower))
  
  return (
    <div className="container mx-auto p-6 space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>PCS 展示（版本2）</CardTitle>
          <CardDescription>功率变换系统实时监控 - 使用EnergyFlowPanel组件</CardDescription>
        </CardHeader>
        <CardContent>
          {/* 设备选择 */}
          <div className="mb-6">
          ...
          <div className="mb-6">
            <EnergyFlowPanel
              title="实时能量流向"
              direction={data.mode === 'charging' ? 'charge' : 'discharge'}
              gridSide={{
                title: 'Grid',
                subtitle: 'AC Side',
                metrics: [
                  { label: 'AC Power', value: Math.abs(data.acActivePower).toFixed(1), unit: 'kW' },
                  { label: 'Voltage AB', value: data.acVoltageAB.toFixed(1), unit: 'V' },
                  { label: 'Voltage BC', value: data.acVoltageBC.toFixed(1), unit: 'V' },
                  { label: 'Voltage CA', value: data.acVoltageCA.toFixed(1), unit: 'V' },
                  { label: 'Frequency', value: data.acFrequency.toFixed(2), unit: 'Hz' },
                  { label: 'Power Factor', value: data.acPowerFactor.toFixed(3), unit: '' },
                ],
              }}
              batterySide={{
                title: 'Battery Array',
                subtitle: 'DC Side',
                metrics: [
                  { label: 'DC Power', value: Math.abs(data.dcPower).toFixed(1), unit: 'kW' },
                  { label: 'Voltage', value: data.dcInputVoltage.toFixed(1), unit: 'V' },
                  { label: 'Bus Voltage', value: data.dcBusVoltage.toFixed(1), unit: 'V' },
                  { label: 'Current', value: Math.abs(data.dcCurrent).toFixed(1), unit: 'A' },
                  { 
                    label: 'SOC', 
                    value: Math.min(100, Math.max(0, ((data.dcInputVoltage - 550) / 100 * 100))).toFixed(0), 
                    unit: '%' 
                  },
                ],
              }}
              pcs={{
                title: 'PCS',
                status: getModeText(data.mode),
                efficiency: efficiency,
                metrics: [
                  { label: 'Conversion Loss', value: conversionLoss.toFixed(1), unit: 'kW' },
                  { label: 'Module Temp', value: data.moduleTemperature.toFixed(1), unit: '°C' },
                  { label: 'Ambient Temp', value: data.ambientTemperature.toFixed(1), unit: '°C' },
                ],
              }}
            />
          </div>
